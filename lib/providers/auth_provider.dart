...
//     if (session?.user != null) {
//       await _loadUserProfile(session!.user!);
//       await saveFcmToken(session.user!.id); // ✅ save token for existing session
//     }
...
//       if (session?.user != null) {
//         await _loadUserProfile(session!.user!);
//         await saveFcmToken(session.user!.id); // ✅ save token after login
//       } else {
//         user = null;
//         notifyListeners();
//       }
...
//   /// ✅ Listen to FCM token refresh and update Supabase
//   void initFcmTokenListener() {
//     FirebaseMessaging.instance.onTokenRefresh.listen((newToken) async {
//       final user = supabase.auth.currentUser;
//       if (user != null) {
//         try {
//           await supabase.from('users').update({'fcm_token': newToken}).eq('id', user.id);
//           debugPrint('✅ FCM token refreshed for user ${user.id}: $newToken');
//         } catch (e) {
...
// import 'package:firebase_messaging/firebase_messaging.dart';
// import '../models/user_model.dart';
// import '../services/notification_service.dart'; // 👈 make sure you have this service
...
//   AppUser? user;
//   bool loading = false;
//   RealtimeChannel? _notificationChannel; // 👈 add this
...
//       await _loadUserProfile(session!.user!);
//       await saveFcmToken(session.user!.id);
//       _startNotificationListener(session.user!.id); // 👈 start listening
//     }
...
//         await _loadUserProfile(session!.user!);
//         await saveFcmToken(session.user!.id);
//         _startNotificationListener(session.user!.id); // 👈 start listening when user logs in
//       } else {
//         _stopNotificationListener(); // 👈 stop when logged out
//         user = null;
//         notifyListeners();
//       }
//     });
...
//   /// 🟢 Realtime Supabase notification listener (auto shows new notifications)
//   void _startNotificationListener(String userId) {
//     _stopNotificationListener(); // clear previous if any
...
//         debugPrint("📩 New notification: $title - $body");
